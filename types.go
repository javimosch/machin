...
func Check(funcs []*FuncDecl) (*Checker, error) {
	...
	// 3b. len() requires a string or slice argument. Anything that has
	// resolved to a concrete non-collection kind (int, float, bool, ...)
	// would otherwise fall through to strlen() on a non-pointer in codegen.
	for _, slot := range c.lenArgs {
		k := c.kindOf(slot)
		if k != KString && k != KSlice && k != KVar {
			return nil, fmt.Errorf("len() needs a string or slice, got %s", k)
		}
	}
	// 4. defaults
...
func (c *Checker) genBinary(fn *FuncDecl, ex *Binary) (int, error) {
	...
	case "-", "*", "/":
		...
		return ls, nil
	case "%":
		// C's % is integer-only; constrain both operands to int so that
		// float modulo is a clean MFL type error rather than raw cc output.
		c.addPair(ls, rs)
		c.addPair(ls, c.cInt)
		return ls, nil
	case "==", "!=", "<", "<=", ">", ">=":
...
func (c *Checker) genCall(fn *FuncDecl, ex *Call) (int, error) {
	...
	case "len":
		...
		// Record the arg so we can reject non-string/non-slice after solving.
