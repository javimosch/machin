...
func (c *Checker) genExprInner(fn *FuncDecl, e Expr) (int, error) {
	...
	case *MakeSlice:
		...
		lslot, err := c.genExpr(fn, ex.Len)
		if err != nil {
			return 0, err
		}
		c.addPair(lslot, c.cInt)
		if ex.Cap != nil {
			cslot, err := c.genExpr(fn, ex.Cap)
			if err != nil {
				return 0, err
			}
			c.addPair(cslot, c.cInt)
		}
...
func (c *Checker) genCall(fn *FuncDecl, ex *Call) (int, error) {
	...
	case "listen_on":
		if len(argSlots) != 2 {
			return 0, fmt.Errorf("listen_on: 2 args (host string, port int)")
		}
		c.addPair(argSlots[0], c.cString)
		c.addPair(argSlots[1], c.cInt)
		return c.cInt, nil
