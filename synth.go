...
// synthesis grammar
var synthConsts = []int64{-1, 0, 1, 2}
var synthOps = []string{"+", "-", "*", "/", "%"}

const (
	synthMaxSize   = 7      // max AST node count of a candidate (n*(n+1)/2 is 7)
	synthExploreCap = 400000 // max candidate terms generated
)
...
func synthesize(target *FuncDecl, params []string, funcs map[string]*FuncDecl, structs map[string]*TypeDecl) synthResult {
	...
	bank := map[int][]Expr{} // size -> representative terms (one per distinct behavior)
	seen := map[string]bool{}
	var best Expr
	bestCost := funcCost(target) // only a STRICTLY cheaper equivalent replaces the original

	consider := func(e Expr) {
		...
		if seen[sig] {
			return // a smaller term already realises this behavior
		}
		seen[sig] = true
	}

	// size 1: parameters and constants
	var size1 []Expr
	for _, p := range params {
		size1 = append(size1, &Ident{Name: p})
	}
	for _, k := range synthConsts {
		size1 = append(size1, &IntLit{Val: k})
	}
	for _, e := range size1 {
		consider(e)
	}
	bank[1] = dedupBySig(size1, sample, pidx)

	for size := 2; size <= synthMaxSize; size++ {
		var produced []Expr
		// unary: -a  (adds one node)
		for _, a := range bank[size-1] {
			e := &Unary{Op: "-", X: a}
			consider(e)
			produced = append(produced, e)
		}
		// binary: a op b  (adds one node; sizes of a and b sum to size-1)
		for i := 1; i <= size-2; i++ {
			j := size - 1 - i
			for _, a := range bank[i] {
				for _, b := range bank[j] {
					for _, op := range synthOps {
						e := &Binary{Op: op, L: a, R: b}
						consider(e)
						produced = append(produced, e)
						if res.Explored >= synthExploreCap {
							break
		...
		bank[size] = dedupBySig(produced, sample, pidx)
	}
...
// dedupBySig keeps one term per distinct sample-domain behavior (the observational-equivalence
// prune that keeps the search tractable), dropping any term that traps.
func dedupBySig(terms []Expr, domain [][]int64, pidx map[string]int) []Expr {
	seen := map[string]bool{}
	var out []Expr
	for _, e := range terms {
		sig, ok := signature(e, domain, pidx)
		if !ok || seen[sig] {
			continue
		}
		seen[sig] = true
