...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Commands: []guideCommand{
			{"run", "machin run <file.mfl> [--safe] [--record <trace>|--replay <trace>] [--verify]", "compile to native and execute in one step. --record <trace> writes a deterministic-replay trace (the goroutine schedule + I/O log); --replay <trace> re-executes it byte-for-byte, feeding the recorded schedule and I/O instead of the real clock/stdin. SOUND because machin proves the program race-free — replay records the channel-op order, not memory accesses. A program that uses FFI or the high-level HTTP/TLS/WebSocket reads (http/https/wss/tls) leaves the determinism boundary, so its trace is flagged best-effort (replay warns, and --verify never certifies it FAITHFUL). `select` is gated (the chosen case is recorded and forced on replay), and rand_bytes, file reads, and raw fd sockets (dial/listen/accept/read/write) are captured, so those stay faithful and self-contained."},
			{"replay", "machin replay <trace> [--json] [--verify]", "re-execute a recorded trace (from `machin run --record`). The trace embeds its program path + build flags, so you replay without re-naming the source; a recorded CRASH reproduces exactly. --json turns a crash into a structured causal report {panic, goroutine, scheduleOp, causalChain:[goroutine-ids of the channel ops that led here]} an agent reads instead of reproduces. --verify certifies the replay stayed faithful (see `run`)."},
