...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"windows-target", "`machin build app.mfl --target windows` cross-compiles to a Windows x86-64 `.exe` via `zig cc -target x86_64-windows-gnu` (needs `zig`; override with ZIG=). Supported (#517 Phases 0/N/TLS/XEdDSA): the POSIX-independent core — stdio, strings/maps/slices, structs, json(), arena blocks + `arena_reset()`, goroutines + channels (mingw-w64's winpthreads), math, file/dir I/O — PLUS TCP sockets (dial/listen/accept/read/write/close/peer_addr/socket_timeout) via winsock2, PLUS HTTPS/TLS (https_*/wss_*) and the OpenSSL crypto builtins (rsa_*/x509_pubkey/rand_bytes/hmac/... — so the whole pure-MFL SAML/OIDC path cross-compiles), PLUS XEdDSA (`xeddsa_sign`/`xeddsa_verify`). TLS/crypto link a mingw OpenSSL the caller supplies via env `MACHIN_WIN_OPENSSL=/path` (a dir with include/ and lib/, e.g. msys2's mingw-w64-x86_64-openssl); XEdDSA additionally needs a mingw libsodium via `MACHIN_WIN_SODIUM=/path` (a dir with lib/ holding libsodium.a, e.g. msys2's mingw-w64-x86_64-libsodium). The CA root bundle is compiled in so TLS certificates verify with no external files. Missing either library produces a clear error naming the requirement (not a cryptic link fail). Still unsupported: terminal raw mode, SQLite, POSIX regex. Portability is by `#ifdef _WIN32` in the emitted runtime, so the same C still builds native/wasm unchanged."},
