...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Commands: []guideCommand{
			...
			{"build", "machin build <file.mfl> [-o out] [--target native|wasm|windows] [--static] [--emit-c] [--safe] [--race-safe]", "compile to a native binary (or a wasm module with --target wasm, mark exports with `export func`; or a Windows x86-64 .exe with --target windows). --static bundles SQLite for a FROM-scratch binary (pair with CC=musl-gcc); --emit-c prints the generated C and stops; --safe inserts bounds/div-zero/overflow checks; --race-safe refuses to build if a data race is inferred (see `check`). The windows target (needs zig; override with ZIG=) cross-compiles the POSIX-independent core — stdio, strings/maps/slices, arena + arena_reset, goroutines/channels/select (winpthreads — and CI now RUNS a goroutine fan-in plus a select on a real windows runner, rather than only compiling one), math, file I/O, plus SQLITE and ZLIB (each compiled in from vendored source rather than needing a library the user must find; SQLite is the same amalgamation the native --static build uses — it carries its own Win32 VFS, so no external DLL; verified by CI running the cross-compiled .exe on a real windows-latest runner, not merely compiling it); TERMINAL RAW MODE (raw_mode/read_key) maps onto the console API (SetConsoleMode + _kbhit/_getch, peeking a redirected stdin so it never blocks) and CI runs the cross-compiled .exe off a tty, where it must match POSIX exactly; it rejects programs using XEdDSA or regex with a #517 message (those are later phases)."},
			...
			{"alias", "machin alias [--json] <src...>", "REPORT ONLY (#578 step 1) — which local slices are provably UNALIASED, and why the rest are not. A slice whose backing array no other live reference observes could be grown in place by `append` instead of the current allocate-fresh-and-memcpy, which is the whole of machin's remaining gap to Rust on the array-heavy sieve (79ms of appends vs Rust's 27ms; an in-place prototype hit parity). NO CODEGEN CONSUMES THIS YET — it exists so the proof is reviewable before anything depends on it. FAIL-CLOSED: every operation on a candidate must be on a whitelist of forms that cannot leak a reference (append-to-self, index read/write, len, range); anything else, INCLUDING an AST node the analysis does not recognise, refuses the slice. The rejected alternative was a dynamic `shared` bit on the slice header, which fails OPEN — a header-copy site nobody thought of leaves an alias unmarked, and an unmarked alias is silent corruption. FLOW-SENSITIVE: an alias created AFTER the last append cannot observe a moved array, so it does not disqualify; that is not a nicety, since a first flow-insensitive version was sound but fired on almost nothing real (\"build it, then use it\" is the dominant shape). Every refusal names the disqualifying construct, so it is auditable."},
			...
			{"test", "machin test [--json] [--cover] <src...>", "run MFL test files: composes framework/test.src ahead of the given sources (same multi-file compose as `encode` — pass a framework module alongside its test file to test it), builds+runs the result as one program, reports the pass/fail tally. framework/test.src provides `assert(cond, msg)`, `assert_eq_int(got, want, name)`, `assert_eq_str(got, want, name)`, and `test_summary()` (call last in main() — prints the tally, exits 1 on any failure). --json returns {ok, passed, failed, files}. One composed program per invocation — run it once per test suite, not once for a whole tree. --cover adds FUNCTION-ENTRY coverage (#589): per source file, which declared functions were entered at least once, plus the names of those that were not. The verdict names its own granularity (\"kind\":\"function\") because entry coverage is much weaker than statement or branch coverage and must never be quoted as either. The DENOMINATOR comes from the parsed AST, not from emitted code: machin only instantiates functions something calls, so counting emitted functions would report 100% while silently excluding every function no test reaches. A function that IS compiled but never entered (called only from a branch that does not run) reports uncovered too — the metric is entry, not instantiation — and a generic's monomorphized instances collapse to the one function you wrote. --cover changes instrumentation only; the pass/fail tally is identical with and without it. --cover also emits a SEPARATE statement-coverage block (\"kind\":\"statement\") listing the functions containing at least one unexecuted statement — because a function can read 100% covered at function level while whole branches go unexercised, and the two numbers answer different questions (\"was this ever called\" vs \"did this code run\"). Never quote the function number as if it were the statement one."},
		...
		Types: []guideNote{
			...
			{"[]T", "slice. make([]T, n) preallocates n ZEROED elements (not n slots of garbage — index straight in); make([]T, len, cap) also reserves capacity, so an append loop never reallocates AT ALL — reach for it whenever you know roughly how many elements you are about to append, since not growing beats the in-place growth of #578. A capacity below the length is raised to the length rather than trusted (a slice whose len exceeds its allocation makes the next index a heap overrun); a negative size clamps to 0. Otherwise append to grow; for i, v := range. T can be a struct, another slice, or `func` (`[]func{}` — a slice of closures, for dispatch tables / effect lists). Slice-range s[lo:hi] / s[lo:] / s[:hi] (lo defaults 0, hi defaults len(s)) ALWAYS returns a fresh COPY, never a view sharing s's backing storage — matches substr's copy semantics on strings and means mutating the result (e.g. via append or index-assign) never affects s. Bounds (0 <= lo <= hi <= len(s)) are checked unconditionally, like s[i] under --safe; a violation panics the same way"},
		...
		Builtins: []guideBuiltin{
			...
			{"arena_reset", "() -> int", "free the CURRENT goroutine's value-arena chain in place, without ending the goroutine — hands all strings/slice-backings/closure-envs allocated so far back to the OS. The escape hatch for a long-running SINGLE-ACTOR server (one that can't run each request in its own goroutine, e.g. a non-thread-safe store): the main goroutine's arena otherwise grows forever, so call arena_reset() at a quiescent point to keep RSS flat. UNCHECKED, unlike an `arena { }` block (whose escape analysis PROVES nothing escapes): you assert NO arena-allocated value is still reachable — a survivor dangles. Keep any cross-reset state in malloc-backed maps/channels or on disk. Prefer `arena { }` when the lifetime is a lexical scope; reach for arena_reset() only across the request loop of a persistent single-actor server. On glibc it also runs malloc_trim(0) so the freed pages are RETURNED TO THE OS (RSS actually drops, not just the C heap free-list); no-op on musl/macOS (their allocators already release freed spans). PERSISTENCE RULE: string literals, env() values, and args() values SURVIVE arena_reset() (their backing memory is not on the arena chain), but COMPUTED values (concatenation, append, a built slice) are CORRUPTED — freed pages may be reused, no crash, exit 0. This applies to LOCALS as well as globals: `x := \"a\" + env(\"T\")` read after a reset comes back as garbage exactly like a global does. ARENA003 now reports it (advisory): a global still holding arena memory at the reset, or a local that is READ after it. Re-derive any computed value from args()/env()/literals after each reset. Returns 0", "memory"},
			{"escape", "(T) -> T", "inside an arena { } block: copy a value into the ENCLOSING arena so it survives the block's bulk reclamation — the one sanctioned way to carry an answer out. Without it a scoped arena is all-or-nothing (useful for pure side-effect work, useless for \"do heavy transient work, return one result\"), which is exactly a request handler in a long-running single-actor server whose goroutine arena never returns. Wrap the work, escape the answer: `body := \"\"  arena { ...heavy...  body = escape(built) }` — everything else is freed, `body` stays valid. ARENA001 treats an escaped value as untainted (it provably outlives the block) and still fires on any OTHER assignment out of the block, so the proof keeps its teeth. Compile error outside an arena block. v1 copies string/bytes (and passes scalars through); build an aggregate into a string (json()/join()) before escaping it. Prefer this over the UNCHECKED arena_reset() whenever the lifetime is a lexical scope", "memory"},
		...
		{"matmul_f32", "(int, int, int, int, int, int, int) -> void", "fp32 batched matmul: (out, x, w, bias, n_in, n_out, batch) -> out[b*n_out+o] = dot(x+b*n_in, w+o*n_in, n_in) + bias[o]. w is [n_out,n_in] row-major. Replaces n_out*batch dot_f32 calls — weight row reused across batch. For QKV/FFN/projection layers", "memory"},
		{"gemm_f32", "(int, int, int, int, int, int, int, int, int) -> void", "fp32 GEMM for training: (c, a, b, m, n, k, ta, tb, accumulate) -> C[m×n] (+)= op(A)[m×k] @ op(B)[k×n], row-major. ta=1 -> A stored [k×m] used transposed; tb=1 -> B stored [n×k]. accumulate=1 adds into C else overwrites. Multithreaded (MFL_GEMM_THREADS, default online CPUs cap 64), cache-blocked MC/KC/NC + 4×8 register-tiled AVX2/FMA micro-kernel (portable scalar fallback), packed A/B panels. Backward passes: dX=dY@W (tb=1), dW=dY^T@X (ta=1). No aliasing C with A/B. matmul_f32(out,x,w,0,n_in,n_out,batch) == gemm_f32(out,x,w,batch,n_out,n_in,0,1,0). Matches naive ref within 1e-4 rel (fp32, accum order differs). Small (m*n*k<64k) runs single-threaded. CPU path even when OpenCL ready (TODO gemm-gpu)", "memory"},
		{"attn_causal_fwd_f32", "(int, int, int, int, int, int, int, int, int, int, int) -> void", "causal multi-head attention forward (MTLM trainer): (q, k, v, probs, out, B, T, dim, kv_dim, heads, kv_heads) -> out. q:[bt,dim] k,v:[bt,kv_dim] (already RoPE'd), out:[bt,dim], probs:[B*heads*T*T] softmax probs saved for backward (layout probs[((b*heads+h)*T+t)*T+s], only s<=t written). scale=1/sqrt(head_size), head_size=dim/heads; GQA: head h uses kv head h/(heads/kv_heads). Multithreaded over (b,head) via the elem pool (MFL_GEMM_THREADS); small runs single-threaded. fp32 accumulation, same op order as mtlm m_attn_fwd_chunk. Replaces the 4.5s/step scalar-MFL attention loop", "memory"},
		{"attn_causal_bwd_f32", "(int, int, int, int, int, int, int, int, int, int, int, int, int, int) -> void", "causal multi-head attention backward (MTLM): (q, k, v, probs, d_out, dq, dk, dv, B, T, dim, kv_dim, heads, kv_heads) -> ACCUMULATES into dq/dk/dv (caller zeroes). d_raw[s]=d_out_t·v_s; d_score=p*(d_raw−Σp*d_raw); dq_t+=scale*d_score*k_s; dk_s+=scale*d_score*q_t; dv_s+=p*d_out_t. Parallel over (b,kv_head) GROUPS so the heads/kv_heads query heads sharing a kv row are one task -> dk/dv writes disjoint (no GQA data race). fp32, same op order as mtlm m_attn_back_chunk", "memory"},
		{"rmsnorm_fwd_f32", "(int, int, int, int, int, int, float) -> void", "RMSNorm forward (MTLM): (out, x, w, normed, n, rows, eps) -> out=w*x*rsqrt(mean(x²)+eps), normed=x*rsqrt(...) (saved for bwd). rows rows of length n. eps=1e-5. Multithreaded over rows. Matches mtlm m_rmsnorm (op order)", "memory"},
		{"rmsnorm_bwd_f32", "(int, int, int, int, int, int, int, int, float) -> void", "RMSNorm backward (MTLM): (dx, dout, w, x, normed, dw, n, rows, eps) -> dx ACCUMULATES (caller may pre-fill residual grad), dw accumulates. dx+=rms*(dout*w−normed*mean(dout*w*normed)); dw+=dout*normed. rms recomputed from x. dw uses per-thread private slabs reduced after (race-free). Matches mtlm m_rmsnorm_back2 (op order)", "memory"},
		{"silu_mul_f32", "(int, int, int, int) -> void", "fused SiLU*MUL forward (MTLM FFN gate): (out, h1, h3, n) -> out=silu(h1)*h3, silu(x)=x/(1+exp(-x)), n elements. Multithreaded over 1024-element chunks. Replaces the scalar MFL gate loop", "memory"},
		{"silu_mul_bwd_f32", "(int, int, int, int, int, int) -> void", "fused SiLU*MUL backward (MTLM FFN gate): (dh1, dh3, dout, h1, h3, n) -> WRITES (overwrite) dh1=dout*h3*silu_grad(h1), dh3=dout*silu(h1); silu_grad(x)=s+x*s*(1−s), s=silu(x). Multithreaded over 1024-element chunks. Matches mtlm m_backward gate grad", "memory"},
		{"softmax_xent_f32", "(int, int, int, int, int, int) -> float", "softmax + cross-entropy (MTLM head): (logits, targets_i32, probs, dlogits, rows, vocab) -> mean cross-entropy over rows; writes probs (stable softmax, max-subtract) and dlogits=(probs−onehot)/rows. targets_i32 is an i32 buffer of row target ids. Multithreaded over rows; double intermediates for the softmax sum + loss to match the MFL reference's loss to 1e-6 (probs/dlogits stored fp32). Matches mtlm m_forward's loss", "memory"},
		{"conv2d_f32", "(int, int, int, int, int, int, int, int, int, int, int, int) -> void", "2D convolution (NCHW): (out, in, w, bias, c_in, c_out, h, w_dim, kh, kw, pad, stride). w is [c_out,c_in,kh,kw]. Auto-vectorized C — replaces scalar MFL conv2d for ResNet/VAE convolutions", "memory"},
		{"add_vec_spatial_f32", "(int, int, int, int) -> void", "Broadcast add vector to spatial tensor (NCHW): (out, vec, channels, hw). out[c,pos] += vec[c] for all positions. GPU dispatch via OpenCL. Replaces scalar MFL peek/poke loop for time-embedding addition in UNet resnet blocks", "memory"},
		{"transpose_chw_f32", "(int, int, int, int, int) -> void", "Transpose [C,H,W] → [H*W,C]: (out, in, channels, h, w). GPU dispatch via OpenCL. Replaces scalar triple-nested MFL loop in attention reshape", "memory"},
		{"transpose_add_chw_f32", "(int, int, int, int, int, int) -> void", "Transpose [H*W,C] → [C,H,W] + add residual: (out, residual, in, channels, h, w). GPU dispatch via OpenCL. Replaces scalar triple-nested MFL loop in attention reshape-back + residual", "memory"},
		{"geglu_f32", "(int, int, int, int) -> void", "GeGLU activation: (out, in, inner, seq). out[i*inner+j] = in[i*inner2+j] * gelu(in[i*inner2+inner+j]). tanh approximation. Vectorized C, replaces scalar MFL loop with exp in transformer feed-forward", "memory"},
		{"concat_chw_f32", "(int, int, int, int, int, int) -> void", "Concatenate two NCHW tensors along channel dim: (out, a, b, a_ch, b_ch, hw). Vectorized C memcpy. Replaces scalar triple-nested MFL loop in UNet up-block skip concatenation", "memory"},
		{"group_norm_f32", "(int, int, int, int, int, int, int, int, float) -> void", "GroupNorm (NCHW): (out, in, weight, bias, channels, h, w_dim, groups, eps). Normalizes each group over (C/G, H, W). Auto-vectorized C — replaces scalar MFL group_norm (VAE/UNet bottleneck at high res)", "memory"},
		{"group_norm_silu_f32", "(int, int, int, int, int, int, int, int, float) -> void", "Fused GroupNorm+SiLU (NCHW): same args as group_norm_f32 but applies SiLU activation in the same GPU kernel pass. Eliminates the separate CPU silu_f32 pass (4-8s saved in VAE). GPU dispatch via OpenCL, CPU fallback", "memory"},
		{"attention_f32", "(int, int, int, int, int, int, int, int, float) -> void", "Fused multi-head attention: (q, k, v, out, seq, ctx_seq, channels, heads, scale). Online softmax (flash-attention-lite). q/k/v: [seq/ctx_seq, channels], out: [seq, channels]. GPU dispatch via OpenCL, CPU fallback. Replaces scalar dot_f32/axpy_f32 attention loops", "memory"},
		{"silu_f32", "(int, int) -> void", "SiLU in-place: x[k] = x[k]/(1+exp(-x[k])) for k<n. Auto-vectorized C — replaces scalar MFL silu_buf", "memory"},
		{"ocl_init", "() -> int", "Initialize OpenCL GPU backend (loads OpenCL.dll, creates context+queue, compiles kernels). Returns 1 on success, 0 on failure. After init, conv2d_f32/matmul_f32/group_norm_f32 dispatch to GPU automatically. Windows only — no-op on other platforms", "memory"},
			...
			{"copy", "(T) -> T", "a value sharing NO backing storage with its argument — the opt-out from MFL's (Go's) sharing rules. A slice is a header, so `b := a` copies the header and shares the elements; so does passing one to a function and assigning a struct that contains one. copy() rebuilds slices, maps and structs RECURSIVELY (a slice inside a struct inside a slice is detached all the way down; a self-referential type terminates). Scalars and strings come back as they are — strings are immutable, so sharing is unobservable. A channel or a function value is refused at compile time, because a copy of either would be the alias you already had. GOTCHA THIS EXISTS FOR: a struct assignment looks like a copy of everything and is not — three shipped bugs in one game codebase came from it, and none of the symptoms (bodies drifting upward each frame, a rematch that was an instant win) pointed anywhere near the cause", "collection"},
			...
			{"http_request", "(string, string, []string, string) -> (int, string, string)", "auth'd HTTP(S): (method, url [http/https], header lines like \"Authorization: Bearer x\", body) -> (status, body, err). err is \"\" on a response, else dns/connect/tls/header — \"header\" means a header line contained CR/LF and was REFUSED (nothing sent). MULTI-ASSIGN ONLY", "net"},
			// websocket
...
func main() { priv := rand_bytes(32)  pub := secp256k1_pubkey(priv)
	...
	srv := listen(8443)
	if srv < 0 { exit(1) } // listen returns -1 when the port can't be bound
...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"json-strings", "framework/json.src is correct JSON string encoding and decoding in pure MFL. json_get returns the RAW token, so a string value comes back quoted AND escaped -- and the obvious way to strip the quotes, substr(raw, 1, len(raw)-1), is wrong for every string that contains one: a newline arrives as the two characters \\ and n, a tab as \\ and t, and a character outside the BMP as a surrogate PAIR that must be recombined. json_unquote(token) gives the real string (it accepts the token with or without its quotes, so it takes json_get output directly); json_quote(s) gives a JSON string literal, quotes included, ready to concatenate into a document; json_str(doc, path) does both in one call. They round-trip for any input, including quotes, backslashes, control bytes and emoji. Reach for these whenever you build JSON by concatenation or read a string field out of json_get -- the naive strip produces documents no parser will read, and the failure is silent until something downstream chokes."},
			{"saml-verify", "framework/xml.src is pure-MFL SAML / XML-DSig signature verification — no native XML library. It ships a minimal XML parser + Exclusive Canonicalization (xml-exc-c14n#, non-comment), and verifies an enveloped RSA-SHA256 signature on top of the x509_pubkey / rsa_verify_jwk_sha256 builtins. `saml_verify(xml_str, trusted_cert_b64) -> ok` returns 1 only if BOTH the reference digest and the signature verify, using the TRUSTED cert you supply out of band (the IdP's known signing cert / metadata) — the cert embedded in the document is deliberately not trusted; it fails closed on anything it doesn't understand. `saml_signing_cert(xml_str)` extracts the embedded <ds:X509Certificate> base64 (for pinning/comparison, not to feed back into verify). Also usable directly: `xml_parse(s)` -> root node index into g_xn, and `c14n_subtree(idx)` for exclusive c14n of any element. Supported profile: one enveloped Reference, exclusive c14n, rsa-sha256, sha256 digest, the five named entities; numeric char refs / CDATA in signed content are out of scope (fail closed). Completes the SAML arm of issue #484 (RSA + x509_pubkey were the earlier pieces)."},
			...
			{"listen-returns-minus-one", "`listen(port)` returns -1 when the port cannot be bound (taken, or privileged and you are not root), printing the reason to stderr — like dial, it does NOT exit, so guard it: `srv := listen(p)  if srv < 0 { ... return }`. Before v0.93.0 it called exit(1) on bind failure, so that guard was dead code in every server ever written against it (#643). Skipping the guard is not silent: `accept` on a negative fd reports and exits rather than spin in your accept loop."},
			...
			{"cstruct-narrow-field-truncation", "A cstruct field declared u8/u16/i8/i16/i32/u32 is held on the MFL side as a full int64 — assigning it an out-of-range value (e.g. int(float(base.r)*1.5) overflowing a u8) does NOT truncate the MFL value, so reading the field back in MFL shows the untruncated number. The truncation happens ONLY in the generated mfl_to_<Struct> C conversion at the FFI call boundary (e.g. (uint8_t)360 == 104), so a plain build silently passes a DIFFERENT value to the C side than str()/println() shows on the MFL side — this is deliberate (checking every FFI struct conversion would cost every call, not just the misbehaving one). Build with --safe to make this observable: mfl_to_<Struct> then range-checks each narrow field and panics with `integer overflow (<ctype> field '<name>')` instead of wrapping silently."},
			{"memory", "Per-goroutine arena, reclaimed in bulk when the goroutine returns; wrap a hot allocating loop in `arena { ... }` to keep peak memory flat, and carry one result out with escape(v). A map made while a scoped or goroutine arena is current is allocated from THAT arena and reclaimed with it, so a transient per-request/per-page map is not a permanent leak; long-lived maps (made on the main arena) keep malloc + per-entry free on delete. Build with --safe for bounds/overflow/div-zero checks."},
			...
			{"windows-target", "`machin build app.mfl --target windows` cross-compiles to a Windows x86-64 `.exe` via `zig cc -target x86_64-windows-gnu` (needs `zig`; override with ZIG=). Supported (#517 Phases 0/N/TLS): the POSIX-independent core — stdio, strings/maps/slices, structs, json(), arena blocks + `arena_reset()`, goroutines + channels (mingw-w64's winpthreads), math, file/dir I/O — PLUS TCP sockets (dial/listen/accept/read/write/close/peer_addr/socket_timeout) via winsock2, PLUS HTTPS/TLS (https_*/wss_*) and the OpenSSL crypto builtins (rsa_*/x509_pubkey/rand_bytes/hmac/... — so the whole pure-MFL SAML/OIDC path cross-compiles). TLS/crypto link a mingw OpenSSL the caller supplies via env `MACHIN_WIN_OPENSSL=/path` (a dir with include/ and lib/, e.g. msys2's mingw-w64-x86_64-openssl); the CA root bundle is compiled in so certificates verify with no external files. Without it, a clear error names the requirement (not a cryptic link fail). Terminal raw mode (raw_mode/read_key) works too: termios has no Windows equivalent, so raw_mode clears ENABLE_LINE_INPUT|ENABLE_ECHO_INPUT via SetConsoleMode and read_key uses _kbhit/_getch, falling back to PeekNamedPipe/ReadFile when stdin is redirected so it never blocks. OFF a tty both targets agree exactly — raw_mode returns -1 and read_key returns immediately — which is what CI pins by running the .exe. Still unsupported: XEdDSA (libsodium) and POSIX regex. NOTE: a windows .exe writes CRLF line endings (msvcrt text-mode stdio translates each \n), so println output is NOT byte-identical to the native build — strip \r before comparing output across targets. Portability is by `#ifdef _WIN32` in the emitted runtime, so the same C still builds native/wasm unchanged."},
			...
			{"int-overflow-wraps", "Signed integer overflow WRAPS two's-complement (matching Go's spec), on every build target and optimization level: `machin build`/`run`/`--target wasm|windows` all compile the emitted C with `-fwrapv`, so a * b or a + b that overflows 64-bit int gives the same wrapped result at -O0 and -O2 rather than being undefined behaviour the C optimizer could fold away (a real bug this fixed: an FNV-1a-style hash's multiply chain collapsed to a constant under -O2 without it, see issue #547). --safe adds runtime checks that PANIC on overflow instead of wrapping — use it when overflow should be a caught bug rather than defined arithmetic."},
		},
	}
}
