...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"memory", "Per-goroutine arena, reclaimed in bulk when the goroutine returns — and NOT before: every string/bytes/slice temporary (substr, concatenation, bytes_sub, a per-item []T + appends) stays resident until then, so a long loop of small allocations in main or in one worker goroutine grows without bound (a tokenizer doing ~250 KB of temporaries per story hit 19 GB and was OOM-killed; the same loop with each story inside an arena block peaked at 43 MB). Wrap the hot allocating loop BODY in `arena { ... }` to keep peak memory flat, carry one result out with escape(v) or write into a pre-existing raw alloc() buffer (not arena memory, so it survives the block). Tripwire: a root arena that passes MFL_ARENA_WARN_MB (default 1024) with no arena block active prints one stderr warning naming this fix (#660); 0 disables it. A map made while a scoped or goroutine arena is current is allocated from THAT arena and reclaimed with it, so a transient per-request/per-page map is not a permanent leak; long-lived maps (made on the main arena) keep malloc + per-entry free on delete. Build with --safe for bounds/overflow/div-zero checks."},
