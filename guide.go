...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Commands: []guideCommand{
			...
			{"falsify", "machin falsify [--json] [--repro <dir>] <src...>|--stdin", "bounded bug-finder: enumerates small concrete inputs and returns the EXACT input that breaks a function (FALS001 index out of range, FALS002 divide/modulo by zero). Unsound-complete — finds bugs, never proves absence — so a clean result means 'no bug within bounds', not 'correct'; the `--json` envelope {ok, counterexamples, findings, coverage:{checked, skipped, allUnknown}} NEVER claims proved. False-positive-free by construction (only fully-modeled concrete paths are reported). --repro <dir> writes one runnable .mfl per finding that panics under --safe at exactly the predicted trap — an auto-promotable regression test."},
