...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Builtins: []guideBuiltin{
			...
			{"matmul_q4_batch", "(int ob, int ostride, int xq, int xs, int wq, int ws, int n, int gs, int B, int lo, int hi) ->", "batched Q4 matmul: B activations x one weight-row range (n/2 packed bytes), weight read once", "memory"},
