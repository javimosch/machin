...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"windows-target", "`machin build app.mfl --target windows` cross-compiles to a Windows x86-64 `.exe` via `zig cc -target x86_64-windows-gnu` (needs `zig`; override with ZIG=). Supported (#517 Phases 0/N/TLS/TTY): the POSIX-independent core — stdio, strings/maps/slices, structs, json(), arena blocks + `arena_reset()`, goroutines + channels (mingw-w64's winpthreads), math, file/dir I/O — PLUS TCP sockets (dial/listen/accept/read/write/close/peer_addr/socket_timeout) via winsock2, PLUS HTTPS/TLS (https_*/wss_*) and the OpenSSL crypto builtins (rsa_*/x509_pubkey/rand_bytes/hmac/... — so the whole pure-MFL SAML/OIDC path cross-compiles), PLUS terminal input (`raw_mode`/`read_key`) via conio's _kbhit/_getch (no extra link dependency). TLS/crypto link a mingw OpenSSL the caller supplies via env `MACHIN_WIN_OPENSSL=/path` (a dir with include/ and lib/, e.g. msys2's mingw-w64-x86_64-openssl); the CA root bundle is compiled in so certificates verify with no external files. A missing library produces a clear error naming the requirement (not a cryptic link fail). Still unsupported: XEdDSA (libsodium), SQLite, POSIX regex, zlib. Portability is by `#ifdef _WIN32` in the emitted runtime, so the same C still builds native/wasm unchanged."},
