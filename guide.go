...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"windows-target", "`machin build app.mfl --target windows` cross-compiles to a Windows x86-64 `.exe` via `zig cc -target x86_64-windows-gnu` (needs `zig`; override with ZIG=), no other toolchain. Supported (#517 Phases 0+N): the POSIX-independent core — stdio, strings/maps/slices, structs, json(), arena blocks + `arena_reset()`, goroutines + channels (mingw-w64's winpthreads), math, file/dir I/O — PLUS TCP sockets (dial/listen/accept/read/write/close/peer_addr/socket_timeout) via winsock2 (auto-links ws2_32). It does NOT yet support TLS/crypto (OpenSSL-for-Windows — so https_*/wss_*/crypto builtins), terminal raw mode, SQLite, or POSIX regex — a program using any of those fails the build with a clear #517 message rather than a link error. Portability is by `#ifdef _WIN32` in the emitted runtime, so the same C still builds native/wasm unchanged."},
