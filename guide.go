...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Commands: []guideCommand{
			...
			{"check", "machin check [--json] <src...>|--stdin", "lex/parse/typecheck + inferred data-race analysis + advisory falsification — no cc, milliseconds — the fast write→check→fix loop. --json returns {ok, errorCount, diagnostics:[{severity, phase, code, message, decl, line, snippet}], warnings:[...]}, exit 0 iff no errors. Branch on the stable `code` (type-mismatch/undefined-name/undefined-field/arity-mismatch/parse-*/no-main/unsupported-construct/...), NOT the message text; `decl` is the function to fix. The `race` phase reports inferred data races with NO annotations (the guarantee Rust needs Send/Sync for): RACE001 write/write, RACE002 read/write, RACE004 use-after-move (value used after `ch <- v` moved it), each with a counterexample in `message`. The `falsify` phase (in `warnings`, advisory — never fails check/build) reports bounded bug counterexamples: FALS001 index out of range, FALS002 divide/modulo by zero, each with the exact failing input. Reserve a full `build` for when you actually need the binary."},
			{"falsify", "machin falsify [--json] [--repro <dir>] [--strict] <src...>|--stdin", "bounded bug-finder: enumerates small concrete inputs and returns the EXACT input that breaks a function (FALS001 index out of range, FALS002 divide/modulo by zero); reaches through calls (interprocedural) and struct fields. Unsound-complete — finds bugs, never proves absence — so a clean result means 'no bug within bounds', not 'correct'; the `--json` envelope {ok, counterexamples, findings, coverage, functions:[{fn, verdict, tried}], bounds:{sliceLenMax, intDomain, callDepth}} NEVER claims proved (verdict is counterexample/clean/unknown/skipped; `bounds` qualifies every clean). False-positive-free by construction (only fully-modeled concrete paths are reported). --repro <dir> writes one runnable .mfl per finding that panics under --safe at exactly the predicted trap — an auto-promotable regression test. --strict exits non-zero on any counterexample (for CI; advisory by default)."},
...
func handle_one(ctx, fd) { tls := tls_accept(ctx, fd)
	...
	tls_close(tls) }`},
		...
		Gotchas: []guideNote{
			...
			{"falsification", "`machin check` (and `machin falsify`) run a bounded falsifier that hands you the EXACT input breaking a function — FALS001 index-out-of-range, FALS002 divide/modulo-by-zero — as an advisory `warnings` entry (phase `falsify`, never fails the build). Unlike the race pass it is unsound-complete: it finds bugs but NEVER proves absence, so a clean result means 'no bug within the bounds' (int/float/string domains, slice length ≤ 3), not 'correct' — don't read silence as a proof. Every reported finding IS a real bug (false-positive-free: only fully-modeled concrete paths are reported; anything touching an unknown call/FFI/unsupported construct is silently skipped as inconclusive, so complex functions may report nothing). `machin falsify --repro <dir>` writes a runnable repro per finding that panics under `--safe` — commit it as a regression test."},
