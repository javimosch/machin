...
func machinGuide() guideCatalog {
	return guideCatalog{
		...
		Commands: []guideCommand{
			...
			{"build", "machin build <file.mfl> [-o out] [--target native|wasm|windows] [--static] [--emit-c] [--safe] [--race-safe]", "compile to a native binary (or a wasm module with --target wasm, mark exports with `export func`; or a Windows x86-64 .exe with --target windows). --static bundles SQLite for a FROM-scratch binary (pair with CC=musl-gcc); --emit-c prints the generated C and stops; --safe inserts bounds/div-zero/overflow checks; --race-safe refuses to build if a data race is inferred (see `check`). The windows target (needs zig; override with ZIG=) cross-compiles the POSIX-independent core — stdio, strings/maps/slices, arena + arena_reset, goroutines/channels (winpthreads), math, file I/O, plus SQLITE and ZLIB (each compiled in from vendored source rather than needing a library the user must find; SQLite is the same amalgamation the native --static build uses — it carries its own Win32 VFS, so no external DLL; verified by CI running the cross-compiled .exe on a real windows-latest runner, not merely compiling it); it rejects programs using tty raw mode, select, XEdDSA or regex with a #517 message (those are later phases)."},
