...
| MongoDB | ✅ | `framework/mongo.src` + `bson.src`: OP_MSG wire protocol + a BSON codec; insert/find/count/drop, docs → JSON → `parse([]T{})`. **Gap left:** auth (SCRAM), doubles, cursor pagination (getMore) |
